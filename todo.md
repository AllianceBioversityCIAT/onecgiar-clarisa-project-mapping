...
- [x] **5. Track admin role activities and changes (audit log)**
  Unified `audit_events` table replaces the project-only `project_audit_events` (rows migrated, old table dropped). `AuditService.record()` wired into every write path across 7 modules: projects (create/update/metadata_update/archive), mappings (create/counter/agree/remove + project lock/reopen), users (create + dynamic role_changed/deactivated/reactivated/reassigned/update), published snapshots, CSV imports, CLARISA sync, and auth.dev_login (security-sensitive). `system` actor role added; mirror writes are best-effort (try/catch + Logger.warn; never break the user action). Bonus fix: `RequestContextService.run()` was previously never wired in the HTTP lifecycle — added `RequestContextMiddleware` and patched `JwtAuthGuard` so AsyncLocalStorage actually populates `userId`/`requestId` (request IDs in audit rows now match Winston log lines). New `/admin/audit-log` page with filter bar (entityType, actorRole, action, date range, free-text search), p-table with server-side pagination, p-drawer detail panel with diff. Project detail page Activity tab re-wired to the unified endpoint. Users admin dialog gets a History tab with "Actions on this user" / "Actions by this user" toggle. Role-scoped visibility: admin sees all, workflow_admin scoped to project/mapping/snapshot, unit_admin scoped to own + project.metadata*. Date-range filter normalises `to` to end-of-day. Known follow-ups (Minor): pagination page index not in URL (D-2); admin reaches Audit Log via Admin sidebar instead of a top-level header pill (D-3).
