...
### Settings (`/settings`)
Singleton system-wide config backing the admin **Settings** page (`/admin/settings`). The `emailEnabled` flag is an admin-visible toggle only — the live notification path is governed by the `NOTIFICATIONS_ENABLED` / `NOTIFICATIONS_DRY_RUN` env flags on `NotificationsModule` (see above). Deadline enforcement on the negotiation workflow is still a follow-up.
