...
| **Program Rep** | Participate in negotiation on mappings to their program: agree, counter-propose allocation, post chat messages, **request removal** (asks the center side; cannot remove unilaterally). Cannot create mappings or lock rounds. |
| **Center Rep** | Initiate mappings for projects in their center, open/counter-propose/agree/remove during negotiation, **accept or decline a program rep's removal request** from the chat thread, post chat messages, **lock** and **reopen** the project round (only when all mappings are agreed and total = 100%). |
