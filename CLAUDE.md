...
**Workflow-admin Final Decision (`admin_decision` status):** `POST /mappings/projects/:projectId/final-decision` (**workflow_admin only**) is the arbiter override. After reading the thread, the admin sets a binding allocation for **every** non-removed mapping; in one pessimistic-locked transaction each mapping is set to that %, moved to **`admin_decision`** status (new `MappingStatus` value — agreed-equivalent, both agreement flags forced true, needs-assistance cleared), gets an appended `ADMIN_DECISION` event (new `NegotiationEventType`, carries the % + shared justification ≥10), and the project is **locked** (`negotiation_locked = true`) with a `LOCKED` event per mapping. Gate: the decision must cover every non-removed mapping exactly once and sum to **exactly 100**. `admin_decision` is treated as agreed-equivalent everywhere lock logic checks `AGREED` (via `isAgreedLike()` — lock gate, auto-lock, `canLock`). Status badge renders **green ("Admin Decision")**. Migration `1789000000000` widens both enum columns. The workflow admin retains their existing per-mapping negotiation abilities, but Final Decision is the intended primary action (it's surfaced as a button in `consolidated-allocation-pane`, workflow_admin + unlocked + ≥1 active mapping).
